...
public class DeadeokHotel {
	...
	private void hotelStart() {
		...
		while (true) {
			...
			case 4: // 업무종료
				...
				return;
			default:
				System.out.println("작업번호를 잘못 입력했습니다. 다시 입력해주세요. ");
			}
		}
	}
	...
	private void checkOut() {
		...
		if (hotelMap.get(roomNum).getGuest() == null) {
			System.out.println(roomNum + "호 객실에는 체크아웃할 손님이 없습니다.");
			return;
		}
		
		// 체크 아웃 작업은 해당 객실의 손님의 이름을 null로 변경하는 것을 말한다.
		String name = hotelMap.get(roomNum).getGuest();	// 현재 손님이름을 구한다.
		hotelMap.get(roomNum).setGuest(null);	// 손님이름을 null로 설정
	...
	private void checkIn() {
		...
		if (!hotelMap.containsKey(roomNum)) {
			System.out.println(roomNum + "호 객실은 존재하지 않습니다.");
			// return;
		} else if (hotelMap.get(roomNum).getGuest() != null) { // 해당 객실에 손님이 있는지 검사
			...
			hotelMap.get(roomNum).setGuest(name);
